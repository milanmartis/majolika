...
/** ---- Spoločná logika: vyrenderuj bloky do STRINGU ---- */
function esc(str: string): string {
  return str.replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]!));
}

function renderInline(node: TextNode | LinkNode): string {
  if (node.type === 'text') {
    let t = esc(node.text ?? '');
  ...
  if (node.type === 'link') {
    const inner = (node.children ?? [])
      .map((c: Node) => renderInline(c as TextNode | LinkNode))
      .join('');
...
function renderNode(node: Node): string {
  switch (node.type) {
    ...
    case 'list-item': {
      ...
      return (li.children ?? []).map((c: Node) => renderNode(c)).join('');
    }
    case 'image': {
    ...
    case 'link':
    case 'text':
      return renderInline(node as TextNode | LinkNode);
    default:
      // neznámy blok – ignoruj
      return '';
  }
}

export function renderBlocksToHtml(content: Node[] | undefined | null): string {
  return (content ?? []).map((n: Node) => renderNode(n)).join('');
}

/** ---- STRING varianta: vhodné pre ďalšie stringové pipe-y (nbspSmallWords, atď.) ---- */
@Pipe({ name: 'blocksHtmlStr', standalone: true })
export class BlocksHtmlStrPipe implements PipeTransform {
  transform(content: Node[] | undefined | null): string {
    return renderBlocksToHtml(content);
  }
}

/** ---- SafeHtml varianta: priame použitie do [innerHTML] (bez ďalších pipe-ov) ---- */
...
export class BlocksHtmlPipe implements PipeTransform {
  ...
  transform(content: Node[] | undefined | null): SafeHtml {
    return this.s.bypassSecurityTrustHtml(renderBlocksToHtml(content));
  }
}
