...
export class ArticleService {
  ...
  getArticleBySlug(slug: string, lang?: string): Observable<Article> {
    ...
    return this.http.get<StrapiResponse<Omit<Article, 'id'>>>(url).pipe(
      map(res => {
        ...
        art.content = blocks.map(block => {
          ...
          if (block.__component === 'blocks.video-block') {
            ...
            if (resolved && !resolved.startsWith('http')) resolved = `${this.base}${resolved}`;
            if (!resolved && vb.url) resolved = vb.url.startsWith('http') ? vb.url : `${this.base}${vb.url}`;
            return { ...vb, url: resolved, caption: vb.caption } as VideoBlock;
          }
          return block;
        }) as ContentBlock[];
